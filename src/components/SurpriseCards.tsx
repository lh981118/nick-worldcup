    ...
    <section className="mx-auto max-w-[1280px] px-4 py-12 sm:px-6 sm:py-20">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <SurpriseCard
          ...
          caption={
            locale === 'zh'
              ? `黑马球队（ELO ${underdogReachesQF.team.elo}）晋级八强。`
              : locale === 'es'
                ? `Underdog (ELO ${underdogReachesQF.team.elo}) llega a cuartos.`
                : `Underdog (ELO ${underdogReachesQF.team.elo}) reaches the quarterfinals.`
          }
        />
        <SurpriseCard
          tone="rose"
          ...
          headline={
            <>
              ...
              <ArrowRight className="inline-block mx-2 h-4 w-4 text-fg-3" />
              <span className="text-rose font-medium">{locale === 'zh' ? '出局' : 'OUT'}</span>
            </>
          }
          caption={
            locale === 'zh'
              ? '世界前十热门球队小组赛出局。'
              : locale === 'es'
                ? 'Top-10 mundial fuera en fase de grupos.'
                : 'World top-10 team exits in the group stage.'
          }
        />
        <SurpriseCard
          tone="violet"
          ...
          caption={
            locale === 'zh'
              ? `潜在大黑马（ELO ${biggestUpset.team.elo}）。`
              : locale === 'es'
                ? `Cenicienta posible (ELO ${biggestUpset.team.elo}).`
                : `Possible Cinderella run (ELO ${biggestUpset.team.elo}).`
          }
        />
      </div>
    </section>
    ...
    >
      <div className={cn('mb-4 flex items-center gap-2', c.text)}>
        ...
        <span className="font-mono text-[10px] uppercase tracking-[0.18em]">
          {locale === 'zh' ? '概率' : locale === 'es' ? 'Probabilidad' : 'Probability'}
        </span>
      </div>
