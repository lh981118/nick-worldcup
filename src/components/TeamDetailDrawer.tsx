...
export function TeamDetailDrawer({ result, resultNoAbsences }: Props) {
  ...
  const data = useMemo(() => {
    ...
    return {
      ...
      groupFixtures: findGroupFixtures(team.id, result, locale),
      noAbs,
    };
    ...
    <Drawer.Root open={open} onClose={closeTeam} direction="right">
      <Drawer.Portal>
        ...
        <Drawer.Content className="fixed right-0 top-0 z-50 flex h-full w-full max-w-[640px] flex-col border-l border-border bg-bg-1/90 backdrop-blur-2xl outline-none">
          <Drawer.Title className="sr-only">{locale === 'zh' ? '球队详情' : 'Team details'}</Drawer.Title>
          {team && data && (
            <>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex items-center gap-3 font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">
                      ...
                      <span>·</span>
                      <span>{locale === 'zh' ? '排名 #' : 'Rank #'}<span className="text-fg-0 tabular">{data.eloRank}</span></span>
                      {data.group && (<><span>·</span><span>{locale === 'zh' ? '小组 ' : 'Grupo '}<span className="text-fg-0">{data.group}</span></span></>)}
                    </div>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto px-6 py-6 space-y-8">
                ...
                <section className="grid grid-cols-2 gap-3">
                  <BestWorstCard
                    tone="gold" label={locale === 'zh' ? '夺冠概率' : 'Probabilidad campeón'} caption={locale === 'zh' ? '最好情况' : 'Mejor caso'}
                    value={data.wasChamp}
                    ci={wilsonCI(result.stageCounts.champion[teamIdx], result.numSimulations)}
                  />
                  <BestWorstCard
                    tone="rose" label={locale === 'zh' ? '小组出局' : 'Fuera en fase de grupos'} caption={locale === 'zh' ? '最差情况' : 'Peor caso'}
                ...
                <section>
                  <div className="mb-3 flex items-baseline justify-between">
                    <h3 className="font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">
                      {locale === 'zh' ? '各阶段晋级概率' : 'Probabilidad de llegar a cada ronda'}
                    </h3>
                    <span className="font-mono text-[9px] text-fg-3">IC 95%</span>
                  </div>
                  <div className="space-y-1.5">
                    <StageBar label={locale === 'zh' ? '32 强' : 'Octavos (R32)'} value={data.reachR32}  count={result.stageCounts.r32[teamIdx]}      n={result.numSimulations} />
                    <StageBar label={locale === 'zh' ? '16 强' : '16vos (R16)'}   value={data.reachR16}  count={result.stageCounts.r16[teamIdx]}      n={result.numSimulations} />
                    <StageBar label={locale === 'zh' ? '8 强' : 'Cuartos'}        value={data.reachQF}   count={result.stageCounts.qf[teamIdx]}       n={result.numSimulations} />
                    <StageBar label={locale === 'zh' ? '半决赛' : 'Semifinal'}      value={data.reachSF}   count={result.stageCounts.sf[teamIdx]}       n={result.numSimulations} />
                    <StageBar label={locale === 'zh' ? '决赛' : 'Final'}          value={data.reachFinal} count={result.stageCounts.final[teamIdx]}    n={result.numSimulations} />
                    <StageBar label={locale === 'zh' ? '冠军' : 'Campeón'}        value={data.wasChamp}  count={result.stageCounts.champion[teamIdx]} n={result.numSimulations} tone="gold" />
                  </div>
                </section>
                ...
                <section>
                  <h3 className="mb-3 font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">
                    {locale === 'zh' ? '小组排名' : 'Posición de grupo'}
                  </h3>
                  <div className="grid grid-cols-5 gap-2 text-center">
                    <GroupCell label="1°"        value={data.groupFirst}    tone="emerald" />
                    <GroupCell label="2°"        value={data.groupSecond}   tone="emerald" />
                    <GroupCell label={locale === 'zh' ? '第 3（晋级）' : '3° (clasif.)'} value={data.groupThirdAdv} tone="amber" />
                    <GroupCell label={locale === 'zh' ? '第 3（出局）' : '3° (out)'} value={data.groupThirdOut} tone="muted" />
                ...
                {/* Goals */}
                <section className="grid grid-cols-2 gap-3">
                  <StatBlock label={locale === 'zh' ? '预期进球（GF）' : 'Goles esperados (GF)'} value={data.avgGF.toFixed(2)} sub={locale === 'zh' ? '每届赛事平均' : 'promedio por torneo'} tone="emerald" />
                  <StatBlock label={locale === 'zh' ? '预期失球（GA）' : 'Goles esperados (GC)'} value={data.avgGA.toFixed(2)} sub={locale === 'zh' ? '每届赛事平均' : 'promedio por torneo'} tone="rose" />
                </section>
                ...
                {data.noAbs && (
                  <section>
                    <h3 className="mb-3 font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">
                      {locale === 'zh' ? '无伤停情景' : 'Escenario sin lesiones'}{' '}
                      <span className="text-fg-3">· {locale === 'zh' ? '假设所有标记球员都能出场' : 'si todos los flagged jugaran'}</span>
                    </h3>
                    <div className="grid grid-cols-2 gap-3 text-center">
                      <DeltaCard
                        label={locale === 'zh' ? '夺冠概率' : 'Probabilidad campeón'}
                        current={data.wasChamp}
                        alt={data.noAbs.wasChamp}
                        tone="gold"
                      />
                      <DeltaCard
                        label={locale === 'zh' ? '进入决赛' : 'Llegar a la final'}
                        current={data.reachFinal}
                        alt={data.noAbs.reachFinal}
                        tone="emerald"
                      />
                      <DeltaCard
                        label={locale === 'zh' ? '进入半决赛' : 'Llegar a semis'}
                        current={data.reachSF}
                        alt={data.noAbs.reachSF}
                        tone="emerald"
                      />
                      <DeltaCard
                        label={locale === 'zh' ? '小组出局' : 'Fuera en grupos'}
                ...
                {data.topScorers.length > 0 && (
                  <section>
                    <h3 className="mb-3 font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">
                      {locale === 'zh' ? '预期射手' : 'Anotadores esperados'} <span className="text-fg-3">· {locale === 'zh' ? '近似' : 'aproximación'}</span>
                    </h3>
                    <ul className="space-y-1.5">
                      {data.topScorers.map(([name, goals]) => (
                        <li key={name} className="flex items-center justify-between rounded-lg border border-border bg-bg-2/30 px-3 py-2 text-sm">
                          ...
                          <span className="font-mono tabular text-fg-0">{goals.toFixed(2)} {locale === 'zh' ? '球' : 'goles'}</span>
                        </li>
                      ))}
                    </ul>
                  </section>
                ...
                {data.groupFixtures.length > 0 && (
                  <section>
                    <h3 className="mb-3 font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">
                      {locale === 'zh' ? '小组赛比赛' : 'Partidos de grupo'}
                    </h3>
                    <ul className="space-y-1.5">
                ...
                <p className="text-[10px] text-fg-3 leading-relaxed">
                  {locale === 'zh' ? '指标基于' : 'Métricas sobre'} {result.numSimulations.toLocaleString()} {locale === 'zh' ? '次 2026 世界杯蒙特卡洛模拟。' : 'simulaciones Monte Carlo del Mundial 2026.'}
                  Anotadores: {locale === 'zh' ? '近似' : 'aproximación'} goal-share, ver <a className="underline decoration-gold/40 underline-offset-2" href="/methodology">{locale === 'zh' ? '方法论' : 'metodología'}</a>.
                </p>
              </div>
            </>
          )}
...
function findGroupFixtures(teamId: string, result: SerializedResult, locale: string) {
  ...
  for (const [key, f] of fixtures) {
    ...
    out.push({
      key,
      opponent: teamDisplayName(opponent, locale),
