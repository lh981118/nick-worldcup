    ...
    <section className="mx-auto max-w-[1280px] px-4 py-12 sm:px-6 sm:py-20">
      <header className="mb-6">
        <div className="font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">
          {locale === 'zh' ? '上一场比赛后' : 'Tras el último partido'}
        </div>
        <h2 className="mt-1 font-display text-3xl font-bold tracking-tight text-fg-0 sm:text-4xl">
          {locale === 'zh' ? '概率如何变化' : 'Cómo cambiaron las probabilidades'}
        </h2>
        <p className="mt-2 text-sm text-fg-2">
          ...
          <span className="mx-2 text-fg-3">·</span>
          {locale === 'zh' ? '对比上一轮模拟时间' : 'comparado con la corrida previa de las'} {new Date(movers.prev.id).toLocaleString('es-AR', { hour: '2-digit', minute: '2-digit', day: 'numeric', month: 'short' })}
        </p>
      </header>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-bg-1/40 font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">
              <th className="px-4 py-3 text-left">{locale === 'zh' ? '球队' : 'Equipo'}</th>
              <th className="px-4 py-3 text-right">{locale === 'zh' ? '之前' : 'Antes'}</th>
              <th className="px-4 py-3 text-right">{locale === 'zh' ? '之后' : 'Después'}</th>
