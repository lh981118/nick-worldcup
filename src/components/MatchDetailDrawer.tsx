    ...
    <Drawer.Root open={open} onClose={closeFixture} direction="right">
      <Drawer.Portal>
        ...
        <Drawer.Content className="fixed right-0 top-0 z-50 flex h-full w-full max-w-[640px] flex-col border-l border-border bg-bg-1/90 backdrop-blur-2xl outline-none">
          <Drawer.Title className="sr-only">{locale === 'zh' ? '比赛详情' : 'Match details'}</Drawer.Title>
          {stats && fixture && (
            <>
              <header className="flex items-start justify-between border-b border-border px-6 py-4">
                <div>
                  ...
                  <h2 className="mt-1 font-display text-2xl font-bold text-fg-0">
                    {teamDisplayName(stats.homeTeam, locale)}
                    <span className="mx-2 text-fg-3">vs</span>
                    {teamDisplayName(stats.awayTeam, locale)}
                  </h2>
                </div>
              ...
              <div className="flex-1 overflow-y-auto px-6 py-6 space-y-8">
                ...
                <section className="flex items-center justify-around rounded-2xl border border-border bg-bg-0/40 p-6">
                  ...
                  <div className="text-center">
                    ...
                    <div className="mt-1 font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">
                      {locale === 'zh' ? '最可能比分' : 'Resultado modal'}
                    </div>
                ...
                {/* W/D/L */}
                <section>
                  <h3 className="mb-3 font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">{locale === 'zh' ? '胜平负概率' : 'Probabilidades'}</h3>
                  <div className="grid grid-cols-3 gap-3">
                    <Probability label={teamDisplayName(stats.homeTeam, locale)} value={stats.winsHome} tone="emerald" />
                    <Probability label={locale === 'zh' ? '平局' : 'Empate'} value={stats.draws} tone="muted" />
                    <Probability label={teamDisplayName(stats.awayTeam, locale)} value={stats.winsAway} tone="violet" />
                  </div>
                </section>

                {/* Heatmap */}
                <section>
                  <h3 className="mb-3 font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">{locale === 'zh' ? '比分分布' : 'Distribución de marcadores'}</h3>
                ...
                <section>
                  <h3 className="mb-3 font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">
                    {locale === 'zh' ? '其他' : 'Otros'} <span className="text-fg-3">· {locale === 'zh' ? '简单估算' : 'estimaciones simples'}</span>
                  </h3>
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <StatLine label={locale === 'zh' ? '角球（主队）' : 'Córners (local)'}    value={stats.cornersHome.toFixed(1)} />
                    <StatLine label={locale === 'zh' ? '角球（客队）' : 'Córners (visit.)'}   value={stats.cornersAway.toFixed(1)} />
                    <StatLine label={locale === 'zh' ? '黄牌（主队）' : 'Amarillas (local)'}  value={stats.yellowsHome.toFixed(1)} />
                    <StatLine label={locale === 'zh' ? '黄牌（客队）' : 'Amarillas (visit.)'} value={stats.yellowsAway.toFixed(1)} />
                    <StatLine label={locale === 'zh' ? 'λ 主队' : 'λ local'}            value={stats.lambdaHome.toFixed(2)} />
                    <StatLine label={locale === 'zh' ? 'λ 客队' : 'λ visitante'}        value={stats.lambdaAway.toFixed(2)} />
                  </div>
                </section>

                <p className="text-[10px] text-fg-3 leading-relaxed">
                  {locale === 'zh' ? '胜平负概率' : 'Probabilidades'} calculadas con el modelo ELO + Poisson sobre {result.numSimulations.toLocaleString()} simulaciones.
...
function ScorersList({ teamId, lambda }: { teamId: string; lambda: number }) {
  const locale = useLocale();
  const otherLabel = locale === 'zh' ? '其他' : 'Otros';
  const scorers = getScorers(teamId).filter((s) => s.name !== otherLabel);
  return (
