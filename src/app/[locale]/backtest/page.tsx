  ...
  if (locale === 'zh') {
    return (
      <article className="relative mx-auto max-w-[1100px] px-6 pt-32 pb-14">
        <Link
          href="/"
          className="mb-8 inline-flex items-center gap-2 font-mono text-xs uppercase tracking-[0.18em] text-fg-3 transition-colors hover:text-fg-1"
        >
        ...
        <header>
          ...
          <p className="mt-4 max-w-2xl text-lg text-fg-1">
            这里用当前模型回放 2014、2018、2022 三届世界杯，并使用赛前年末 ELO 与 192 场常规时间赛果来检验模型表现。
          </p>
        </header>
          ...
          <div className="overflow-x-auto rounded-2xl border border-border glass">
            <table className="min-w-full">
              <thead className="font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">
                <tr>
                  <th className="px-4 py-3 text-left">世界杯</th>
                  ...
                  <th className="px-3 py-3 text-right">准确率</th>
                </tr>
              </thead>
              <tbody>
        ...
        <Section title="模型校准">
          <p className="mb-4 text-sm text-fg-2">
            当模型给出某个结果的概率时，真实发生频率是否接近这个概率。点越靠近对角线，说明校准越好。
          </p>
          <ZhCalibrationPlot buckets={r.calibration} />
        </Section>

        <Section title="近期状态权重">
          <p className="mb-4 max-w-3xl text-sm text-fg-2">
            回测显示，加入最近一年状态变化的小权重后，Brier 分数变化很小，但 Top-1 准确率提升约 1.6 个百分点。
            当前生产模型使用 alpha=0.2、lookback=1 年。
          </p>
          <ZhRecentFormHeatmap cells={recentSweep} />
        </Section>

        <Section title="点球模型">
          <p className="mb-4 max-w-3xl text-sm text-fg-2">
            淘汰赛常规时间打平后，系统不再使用完全随机的 50/50，而是根据历史点球大战胜率做贝叶斯收缩估计。
            由于回测样本只有 {penEval.rows.length} 场点球大战，结论仍然偏谨慎。
          </p>
        ...
        <Section title="主场加成">
          <p className="mb-4 max-w-3xl text-sm text-fg-2">
            当前模型在小组赛给东道主增加 100 ELO。历史回测里主队样本较少，因此这个参数更适合作为可调项继续观察。
          </p>
          <ZhSweepChart points={sweep} />
        </Section>

        <Section title="如何理解这些指标">
          <ul className="space-y-3 leading-relaxed text-fg-1">
            <Bullet><strong>Brier 分数</strong>衡量概率预测的均方误差，越低代表概率越可靠。</Bullet>
            <Bullet><strong>Log loss</strong>会更重地惩罚过度自信但错误的预测，是预测市场常用指标。</Bullet>
            <Bullet><strong>Top-1 准确率</strong>表示概率最高的结果最终命中的比例，三选一比赛里平局本身就很难预测。</Bullet>
            <Bullet>这个回测只评估 90 分钟常规时间结果，进入加时或点球的比赛按常规时间平局计算。</Bullet>
          </ul>
        </Section>

        <footer className="mt-16 border-t border-border pt-8 font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">
          数据 · ELO 来自 eloratings.net · 历史赛果来自 openfootball/worldcup.json
        </footer>
      </article>
    );
  }
...
function ZhRecentFormHeatmap({ cells }: { cells: RecentFormSweepCell[] }) {
  ...
  const colorFor = (b: number) => {
    ...
    const t = (b - minBrier) / (maxBrier - minBrier);
    return t < 0.5
      ? `oklch(0.65 0.15 155 / ${(0.55 - t * 0.7).toFixed(2)})`
      : `oklch(0.62 0.20 18 / ${(0.20 + (t - 0.5) * 0.9).toFixed(2)})`;
  };
  ...
  return (
    <div className="overflow-x-auto rounded-2xl border border-border glass p-4">
      <table className="mx-auto font-mono text-xs tabular">
        <thead>
          <tr>
            <th className="px-3 py-2 text-fg-3 text-[10px] uppercase tracking-[0.18em]">回看年限 / alpha</th>
            {alphas.map((a) => (
        ...
        <tbody>
          {lookbacks.map((lb) => (
            <tr key={lb}>
              <td className="px-3 py-2 text-fg-3 text-[10px] uppercase tracking-[0.18em]">{lb} 年</td>
              {alphas.map((a) => {
                const c = cellAt(a, lb);
                const isBest = c === best;
                return (
                  <td key={a} className="px-2 py-1 text-center">
                    <div
                      className={cn('rounded-lg px-3 py-3 transition-all', isBest && 'ring-2 ring-emerald shadow-[0_0_24px_-4px_oklch(0.72_0.17_155/0.6)]')}
      ...
      <div className="mt-3 flex justify-center gap-6 font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">
        <span className="flex items-center gap-2"><span className="h-2 w-4 rounded-sm" style={{ background: 'oklch(0.65 0.15 155 / 0.55)' }} />更好</span>
        <span className="flex items-center gap-2"><span className="h-2 w-4 rounded-sm" style={{ background: 'oklch(0.62 0.20 18 / 0.55)' }} />更差</span>
        <span className="text-fg-2">最优：alpha={best.alpha}, 回看={best.lookbackYears} 年</span>
      </div>
    </div>
  );
}

function ZhSweepChart({ points }: { points: SweepPoint[] }) {
  ...
  const path = (pts: SweepPoint[]) => pts.map((p, i) => `${i === 0 ? 'M' : 'L'} ${x(p.hostBonus).toFixed(1)} ${y(p.hostOnly.brier).toFixed(1)}`).join(' ');
  ...
  return (
    <div className="rounded-2xl border border-border glass p-6">
      <svg viewBox={`0 0 ${W} ${H}`} className="h-[280px] w-full">
        ...
        <text x={(W + PAD_L - PAD_R) / 2} y={H - 6} fontSize={11} fontFamily="JetBrains Mono, monospace" fill="oklch(0.46 0.02 180)" textAnchor="middle">主场加成（ELO 点数）</text>
        <text x={14} y={(H - PAD_B + PAD_T) / 2} fontSize={11} fontFamily="JetBrains Mono, monospace" fill="oklch(0.46 0.02 180)" textAnchor="middle" transform={`rotate(-90 14 ${(H - PAD_B + PAD_T) / 2})`}>Brier（主队样本）</text>
        <path d={path(series.groupOnly)} stroke="oklch(0.76 0.13 180)" strokeWidth={2} fill="none" />
        {series.groupOnly.map((p) => <circle key={`g-${p.hostBonus}`} cx={x(p.hostBonus)} cy={y(p.hostOnly.brier)} r={3.5} fill="oklch(0.76 0.13 180)" />)}
        <path d={path(series.plusKO)} stroke="oklch(0.65 0.22 18)" strokeWidth={2} fill="none" strokeDasharray="4 3" />
        {series.plusKO.map((p) => <circle key={`k-${p.hostBonus}`} cx={x(p.hostBonus)} cy={y(p.hostOnly.brier)} r={3.5} fill="oklch(0.65 0.22 18)" />)}
        <line x1={x(100)} x2={x(100)} y1={PAD_T} y2={H - PAD_B} stroke="oklch(0.62 0.018 180)" strokeDasharray="2 3" strokeWidth={1} />
        <text x={x(100) + 4} y={PAD_T + 10} fontSize={9} fontFamily="JetBrains Mono, monospace" fill="oklch(0.62 0.018 180)">当前值 (100)</text>
      </svg>
      <div className="mt-2 flex justify-center gap-6 font-mono text-[10px] uppercase tracking-[0.18em] text-fg-2">
        <span className="flex items-center gap-2"><span className="h-0.5 w-5 bg-gold" />仅小组赛</span>
        <span className="flex items-center gap-2"><span className="h-0.5 w-5 bg-rose" style={{ borderTop: '1px dashed' }} />小组赛 + 淘汰赛</span>
      </div>
    </div>
  );
}

function ZhCalibrationPlot({ buckets }: { buckets: CalibrationBucket[] }) {
  ...
  return (
    <div className="rounded-2xl border border-border glass p-6">
      <svg viewBox={`0 0 ${W} ${H}`} className="mx-auto h-[320px] w-full max-w-[400px]">
        ...
        <text x={W / 2} y={H - 4} fontSize={10} fontFamily="JetBrains Mono, monospace" fill="oklch(0.46 0.02 180)" textAnchor="middle">预测概率</text>
        <text x={10} y={H / 2} fontSize={10} fontFamily="JetBrains Mono, monospace" fill="oklch(0.46 0.02 180)" textAnchor="middle" transform={`rotate(-90 10 ${H / 2})`}>实际频率</text>
        {buckets.filter((b) => b.count > 0).map((b, i) => {
          const r = Math.max(3, Math.min(12, Math.sqrt(b.count) * 0.6));
          return <circle key={i} cx={x(b.predicted)} cy={y(b.observed)} r={r} fill="oklch(0.76 0.13 180 / 0.7)" stroke="oklch(0.76 0.13 180)" strokeWidth={1} />;
        })}
      </svg>
      <p className="mt-3 text-center font-mono text-[10px] uppercase tracking-[0.18em] text-fg-3">
        每个圆点代表一个概率分桶，圆点越大表示样本越多。
      </p>
    </div>
  );
}
